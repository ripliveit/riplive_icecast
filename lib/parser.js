...
exports.init = function() {
    ...
    ? 'http://www.riplive.it/api/search?post_type=songs&count=1&'
...
exports.attach = function(opts) {
    ...
    this.parse = function(string, cb) {
        //var string = "StreamTitle='Macklemore & Ryan Lewis - Can't Hold Us (feat. Ray Dalton)'";
        var string = string.replace(/'/g, ' ').split(';');
        var array = string[0].split('=');
        var song = array[1].split('-').map(function(item) {
            return item.trim();
        });
